""" Command Line tool to handle XSV """
import argparse
import sys
import MySQLdb
import pandas as pd
...
class Table2XSV(object):
    """ Class to convert Sheet Data to XSV. """

    def __init__(self, sep, encoding):
        self.__sep = sep
        self.__index = False
    ...
    def csv2xsv(self, **kwargs):
        if kwargs["path"].endswith(".csv"):
            ...
            _df.to_csv("{}.csv".format(kwargs["path"]), index=self.__index, sep=self.__sep, encoding=self.__encoding)
        else:
    ...
    def mysql2xsv(self, **kwargs):
        ...
        _connection.close()
        _df.to_csv("{}.csv".format(kwargs["file"]), index=self.__index, sep=self.__sep, encoding=self.__encoding)
...
if __name__ == "__main__":
    try:
        ...
        t2xsv = Table2XSV(args.sep, args.encoding)
        ...
        if args.params[0].lower() == "csv":
            ...
            if len(args.params) == 8:
                t2xsv.mysql2xsv(host=args.params[1], port=args.params[2], user=args.params[3], password=args.params[4],
                                db=args.params[5], query=args.params[6], file=args.params[7])
            else:
                print(else_print)
        else:
